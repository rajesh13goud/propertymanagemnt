...
class MetaPayComponent extends Component {
  ...
  updateName(e) {
    this.setState({ type: "UPDATE_NAME", result: e.target.value });
  }
  updateAddress(e) {
    this.setState({ type: "UPDATE_ADDRESS", result: e.target.value });
  }
  updateCity(e) {
    this.setState({ type: "UPDATE_CITY", result: e.target.value });
  }
  updateCountry(e) {
    this.setState({ type: "UPDATE_COUNTRY", result: e.target.value });
  }
          ...
          <center>
            ...
            <div>
              ...
              <Button
                bsStyle="primary"
                bsSize="large"
                onClick={this.sendTxn.bind(this)}
              >
  ...
  renderCheckout(network_err) {
    ...
    if (network_err) {
      wrongNetwork = (
        <h5>
          We detect metamask, but you're on the wrong network. Please switch to
          Ethereum Main Net
        </h5>
      );
    }
